...
namespace Business.Implementations.Jwt
{
    public class TokenBusiness : IToken
    {
        ...
        public TokenBusiness(IConfiguration configuration, IUserData userData, IRolUserData rolUserData,
            ILogger<TokenBusiness> logger, IRefreshTokenData refreshToken, IOptions<JwtSettings> jwtSettings)
        {
        ...
        public async Task<(string AccessToken, string RefreshToken, string CsrfToken)> GenerateTokensAsync(LoginRequestDto dto)
        {
            ...
            dto.Password = EncriptPassword.EncripteSHA256(dto.Password);
        ...
        public async Task<(string NewAccessToken, string NewRefreshToken)> RefreshAsync(string refreshTokenPlain, string remoteIp = null)
        {
            ...
            var user = await _userData.GetById(record.UserId)
                ?? throw new SecurityTokenException("Usuario no encontrado");

            var roles = await _rolUserData.GetRolesUser(user.Id);
